...
public class RemoveMemberFrame extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					RemoveMemberFrame frame = new RemoveMemberFrame(new ClientMsg("localHost", 1666));
	 ...
	 * Create the frame.
	 */
	public RemoveMemberFrame(ClientMsg c) {
		...
		JLabel titre = new JLabel("Retirer un membre d'un groupe");
		titre.setHorizontalAlignment(SwingConstants.CENTER);
		contentPane.add(titre, BorderLayout.NORTH);
		
		JButton ajouter = new JButton("Retirer ce membre");
		...
		ajouter.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if( nom.getText().length() > 0 && membreId.getText().length()>0) {
					...
					// on met le type de paquet ( 5 pour retirer membre )
					...
					JOptionPane.showMessageDialog(null, "Membre supprimé du groupe si vous êtes le créateur du groupe");
					fermer();
				}
			}
		});
